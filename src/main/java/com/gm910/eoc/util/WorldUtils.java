...
public class WorldUtils {
	...
	public static BlockPos centerCP(Collection<ChunkPos> others) {
		return BlockPos.containing(others.stream().map((c) -> c.getMiddleBlockPosition(0)).map((bp) -> new Vec3(bp))
	...
	public static BlockPos center(Collection<BlockPos> others) {
		return BlockPos.containing(others.stream().map((bp) -> new Vec3(bp)).reduce(Vec3.ZERO, (a, b) -> a.add(b))
